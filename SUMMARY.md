# Table of contents

## OS基礎 <a href="#os-essential" id="os-essential"></a>

* [Linux核心簡介](README.md)
* [Page 2](os-essential/page-2.md)
* [開機流程](os-essential/computer-start.md)
* [BIOS](os-essential/bios.md)
* [記憶體管理(memory management)](os-essential/memory-management/README.md)
  * [分頁(paging)](os-essential/memory-management/fen-ye-paging.md)
  * [虛擬記憶體(virtual memory)](os-essential/memory-management/virtual-memory.md)
  * [記憶體置換(memory swapping)](os-essential/memory-management/ji-yi-ti-zhi-huan-memory-swapping.md)
  * [連續記憶體分配(contiguous memory allocation)](os-essential/memory-management/contiguous-memory-allocation.md)
* [保護環(protection rings)](os-essential/protection-rings.md)
* [NUMA架構](os-essential/numa.md)
* [CPU快取一致性協議MESI](os-essential/cpu-kuai-qu-yi-zhi-xing-xie-yi-mesi.md)
* [中斷(interrupt)](os-essential/interrupt.md)
* [系統呼叫(system call)](os-essential/xi-tong-hu-jiao-system-call.md)
* [Windows API](os-essential/windows-api.md)
* [GLIBC](os-essential/glibc.md)
* [IO](os-essential/io.md)
* [Init行程：sysvinit](os-essential/init-process-sysvinit/README.md)
  * [init行程：upstart](os-essential/init-process-sysvinit/init-process-upstart.md)
  * [init行程：systemd](os-essential/init-process-sysvinit/init-process-systemd.md)

## 韌體 <a href="#firmware" id="firmware"></a>

* [BIOS](firmware/bios.md)
* [UEFI](firmware/uefi.md)
* [EDK2](firmware/edk2.md)

## 虛擬機 <a href="#virtual_machine" id="virtual_machine"></a>

* [在 Linux 啟動 UEFI Secure Boot 導致不能 VirtualBox 不能起動虛擬機](virtual\_machine/zai-linux-qi-dong-uefi-secure-boot-dao-zhi-bu-neng-virtualbox-bu-neng-qi-dong-xu-ni-ji.md)
